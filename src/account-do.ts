...
export class AccountDurableObject extends DurableObject<PDSEnv> {
	...
	async rpcSaveAuthCode(
		code: string,
		data: import("./oauth-provider").AuthCodeData,
	): Promise<void> {
	...
	async rpcGetAuthCode(
		code: string,
	): Promise<import("./oauth-provider").AuthCodeData | null> {
		const storage = await this.getOAuthStorage();
	...
	/** Save token data */
	async rpcSaveTokens(
		data: import("./oauth-provider").TokenData,
	): Promise<void> {
		const storage = await this.getOAuthStorage();
	...
	async rpcGetTokenByAccess(
		accessToken: string,
	): Promise<import("./oauth-provider").TokenData | null> {
		const storage = await this.getOAuthStorage();
	...
	async rpcSaveClient(
		clientId: string,
		metadata: import("./oauth-provider").ClientMetadata,
	): Promise<void> {
		const storage = await this.getOAuthStorage();
	...
	async rpcGetClient(
		clientId: string,
	): Promise<import("./oauth-provider").ClientMetadata | null> {
		const storage = await this.getOAuthStorage();
	...
	async rpcSavePAR(
		requestUri: string,
		data: import("./oauth-provider").PARData,
	): Promise<void> {
		const storage = await this.getOAuthStorage();
	...
	async rpcGetPAR(
		requestUri: string,
	): Promise<import("./oauth-provider").PARData | null> {
		const storage = await this.getOAuthStorage();
	...
	async rpcSavePermissionSet(
		nsid: string,
		set: import("./oauth-provider").LexiconPermissionSet,
	): Promise<void> {
		const storage = await this.getOAuthStorage();
